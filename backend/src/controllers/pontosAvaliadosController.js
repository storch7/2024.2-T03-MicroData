...
// Função assíncrona para buscar um ponto avaliado específico pelo ID
const getPontoAvaliadoById = async (req, res) => {
  try {
    const { id } = req.params; // Obtém o ID dos parâmetros da requisição

    // Recupera o ponto avaliado correspondente ao ID e verifica se está ativo
    const ponto = await prisma.pontosAvaliados.findFirst({
      where: {
        id: parseInt(id), // Converte o ID para inteiro
        ativo: true, // Garante que o ponto avaliado está ativo
      },
    });

    // Verifica se o ponto avaliado foi encontrado e está ativo
    if (!ponto) {
    ...
    res.status(500).json({ error: 'Erro ao buscar ponto avaliado.' });
  }
};
